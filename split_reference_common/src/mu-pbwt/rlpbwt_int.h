...
public:
  ...
  explicit rlpbwt_int(std::string fref, std::vector<std::string> &panel,
    ...
    if (common)
      vrb.bullet("Using only common variants");
   ...
   */
  explicit rlpbwt_int(bcf_srs_t *sr, const std::string &fref,
                      ref_haplotype_set &H, variant_map &V,
                      unsigned int n_ref_samples, bool keep_mono) {
    tac.clock();

    if (sr == nullptr)
      vrb.error("Error reading reference file");
    std::cout << bcf_hdr_nsamples(sr->readers[0].header) * 2 << "\n";
    this->height = n_ref_samples * 2;
    ...
    // Report
    vrb.bullet("mu-PBWT building done (" +
               stb.str(tac.rel_time() * 1.0 / 1000, 2) + "s)");
    vrb.bullet(stb.str(zeros) + " zeros | " + stb.str(ones) + " ones");
  }
