...
//                       sdsl::int_vector<> &div) {
    ...
    explicit rlpbwt_int(std::string fref,
        ...
        if (bcf_sr_set_regions(sr, region.c_str(), 0) == -1)
            vrb.error("Impossible to jump to region [" + region + "] in [" +
                      fref +
                      "]");
        if (bcf_sr_set_targets(sr, region.c_str(), 0, 0) == -1)
            vrb.error(
                    "Impossible to set target region [" + region + "] in [" +
                    fref +
                    "]");
        ...
        while (bcf_sr_next_line(sr)) {
            ...
            rAC = bcf_get_info_int32(sr->readers[0].header, line_ref, "AC",
                                     &vAC, &nAC);
            rAN = bcf_get_info_int32(sr->readers[0].header, line_ref, "AN",
                                     &vAN, &nAN);
            if ((nAC != 1) || (nAN != 1))
                vrb.error(
...
#else
            ngt_ref = bcf_get_genotypes(sr->readers[0].header, line_ref,
...
//            }
            ...
            if (i_site == 0) {
                ...
                if (gt_arr_ref == nullptr || ngt_ref == 0)
                    vrb.error(
                            "Error setting ploidy while reading the first record.");
                ...
                if (max_ploidy_ref == 1) {
                    ...
                    for (int i = 0; i < n_ref_samples; ++i) {
                        if (gt_arr_ref[max_ploidy_ref * i + 1] ==
            ...
            if ((cref != V.vec_pos[i_site]->cref) ||
                ...
                vrb.error(
                        "AC/AN INFO fields in VCF are inconsistent with GT field, update the values in the VCF");
...
//            std::cout << "curr_pos " << curr_pos << "\n";
    ...
    //std::pair<std::vector<std::pair<unsigned int, unsigned int>>, std::vector<std::pair<unsigned int, unsigned int>>>
    std::pair<std::vector<unsigned int>, std::vector<unsigned int>>
    get_similar_haplos(unsigned int index,
            unsigned int col,
            unsigned int row,
        ...
        char curr_s = get_next_char(this->cols[col].zero_first,
                                    index_to_run(index, col));
...
//        }
        ...
        //std::cout << "index: " << index  << " end_u: " << end_u << " end_d: " << end_d << "\n";
...
//                                      this->phi->plcp(start_row, curr_col + 1));
//            haplos_u.emplace_back(up_row,
