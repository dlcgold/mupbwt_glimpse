...
public:
  ...
  explicit rlpbwt_int(std::string fref, std::vector<std::string> &panel,
                      ref_haplotype_set &H, variant_map &V, std::string &region,
                      unsigned int nthreads = 1, bool verbose = false,
                      bool macs = false) {
    tac.clock();
    auto keep_mono = false;
    ...
    if (!(bcf_sr_add_reader(sr, fref.c_str()))) {
      // we do not build an index here, as the target and reference panel could
    ...
    // bcf1_t *rec = bcf_init();
    auto n_ref_samples = bcf_hdr_nsamples(sr->readers[0].header);

    // this->height = n_ref_samples * 2 + panel.size();
    this->height = n_ref_samples * 2 + panel.size();
    this->width = 0;
    ...
    while (bcf_sr_next_line(sr)) {
      ...
      rAC =
          bcf_get_info_int32(sr->readers[0].header, line_ref, "AC", &vAC, &nAC);
      rAN =
          bcf_get_info_int32(sr->readers[0].header, line_ref, "AN", &vAN, &nAN);
      if ((nAC != 1) || (nAN != 1))
        vrb.error(
      ...
      //            int max_ploidy = ngt_ref / n_ref_samples;
      //
      ...
      if (i_site == 0) {
        ...
        if (max_ploidy_ref < 1 || max_ploidy_ref > 2 ||
          ...
          vrb.error("Max ploidy of the reference panel cannot be set to "
                    "neither 1 or 2.");
      ...
      last_col = new_column;
      count++;
      if ((cref != V.vec_pos[i_site]->cref) ||
          (calt != V.vec_pos[i_site]->calt))
    ...
    this->phi =
        new phi_ds(this->cols, this->height, this->width, this->last_pref,
                   this->last_div, supp_b, supp_e, false);
  ...
  explicit rlpbwt_int(std::vector<std::string> &panel) {
    tac.clock();
    vrb.bullet("building mu-PBWT query ");

    this->height = panel.size();
    this->width = panel[0].size();
    ...
    for (unsigned int k = 0; k < panel[0].size(); k++) {

      // std::cout << "Analyze site: " << k << "\r";
      new_column = "";
      for (auto &i : panel) {
        new_column += i[k];
      }
      auto col =
          rlpbwt_int::build_column(new_column, pref, div, supp_b, supp_e, k);
      // this->cols[count] = col;
      this->cols.emplace_back(col);
      rlpbwt_int::update(new_column, pref, div);
      last_col = new_column;
    ...
    vrb.bullet("mu-PBWT query building (" +
  ...
  get_similar_haplos_len(unsigned int index, unsigned int col, unsigned int row,
                         unsigned int len, unsigned int pbwt_depth) {
    // std::vector<std::pair<unsigned int, unsigned int>> haplos_d;
    // std::vector<std::pair<unsigned int, unsigned int>> haplos_u;
    std::vector<unsigned int> haplos_d;
    std::vector<unsigned int> haplos_u;
    std::vector<std::pair<unsigned int, unsigned int>> haplos_l;
    if (row == height) {
      row = height - 1;
    }
    char curr_s =
        get_next_char(this->cols[col].zero_first, index_to_run(index, col));
    // std::cout << "index_old: " << index_old << " ";
    index = lf(col, index, curr_s);
    // unsigned int offset_u = 0;
    // unsigned int offset_d = 0;

    //        int end_u = 0;
    //        int end_d = 0;
    //        end_u = index - pbwt_depth;
    //        end_d = index + pbwt_depth;
    //        if (end_u < 0) {
    //            end_u = 0;
    //        }
    //        if (end_d >= height) {
    ...
    // std::cout << "index: " << index  << " end_u: " << end_u << " end_d: " <<
    // end_d << "\n"; std::cout <<  pbwt_states[sample].size() << "\n";
    unsigned int start_row = row;
    unsigned int depth = 0;
    // haplos_d.emplace_back(row, len);
    // haplos_u.emplace_back(row, len);
    // pbwt_states[sample][depth].push_back((int) start_row);
    // depth++;
    ...
    // haplos_l.push_back({row, len});

    // down
    // if (index != height - 1) {
    // unsigned int down_index = index + 1;
    // while (down_index < end_d || depth <= pbwt_depth - 1) {

    while (depth <= pbwt_depth - 1) {
      // std::cout << "down: " << down_index << " vs " << end_d << "\n";
      auto phi_res = this->phi->phi_inv(start_row, curr_col + 1);
      ...
      // if (tmp_len > len) {
      //   haplos_l.push_back({down_row, len});
      // } else {
      //   if (tmp_len > 1)
      //     haplos_l.push_back({down_row, tmp_len});
      // }
      // haplos_d.emplace_back(down_row,
      //                       this->phi->plcp(down_row, curr_col + 1));
      //            haplos_d.emplace_back(down_row,
      //                                  this->phi->plcp(down_row, curr_col));
      // haplos_d.emplace_back(down_row);
      // pbwt_states[sample][depth].push_back((int) down_row);
      start_row = down_row;
      // down_index++;
    ...
    //}
    // if (index != 0) {
    // unsigned int up_index = index - 1;
    start_row = row;
    depth = 0;

    // up
    // while (up_index >= end_u || depth <= pbwt_depth - 1) {
    while (depth <= pbwt_depth - 1) {
      // std::cout << "up: " << up_index << " vs " << end_u << "\n";
      auto phi_res = this->phi->phi(start_row, curr_col + 1);
      ...
      // if (tmp_len > len) {
      //   haplos_l.push_back({up_row, len});
      // } else {
      //   if (tmp_len > 1)
      //     haplos_l.push_back({up_row, tmp_len});
      // }

      //                haplos_u.emplace_back(up_row,
      //                                      this->phi->plcp(start_row,
      //                                      curr_col + 1));
      //            haplos_u.emplace_back(up_row,
      //                                  this->phi->plcp(start_row, curr_col));
      // haplos_u.emplace_back(up_row);
      // pbwt_states[sample][depth].push_back((int) up_row);
      start_row = up_row;
      // up_index--;
    ...
    //}
    std::sort(haplos_l.begin(), haplos_l.end(),
              [](const auto &a, const auto &b) { return a.second > b.second; });
    std::pair<std::vector<unsigned int>, std::vector<unsigned int>> res;
    // res.first.push_back(row);
    // res.second.push_back(row);

    for (unsigned int i = 0; i < haplos_l.size(); i += 2) {
      res.first.push_back(haplos_l[i].first);
      if (i + 1 < haplos_l.size())
        res.second.push_back(haplos_l[i + 1].first);
    }
    return res;
  }

  // std::pair<std::vector<std::pair<unsigned int, unsigned int>>,
  // std::vector<std::pair<unsigned int, unsigned int>>>
  std::pair<std::vector<unsigned int>, std::vector<unsigned int>>
  get_similar_haplos(unsigned int index, unsigned int col, unsigned int row,
                     unsigned int pbwt_depth) {
    // std::vector<std::pair<unsigned int, unsigned int>> haplos_d;
    // std::vector<std::pair<unsigned int, unsigned int>> haplos_u;
    std::vector<unsigned int> haplos_d;
    std::vector<unsigned int> haplos_u;
    ...
    //}
    return std::make_pair(haplos_d, haplos_u);
  }

  /**
   * @brief function to compute queries with thresholds from a transposed tsv or
   * vcf file and output them on a file
...
#pragma omp parallel for default(none) shared(                                 \
  ...
  std::string get_row(unsigned int row) {

    std::string c = "";
    unsigned int i = row;
    for (unsigned int col = 0; col < this->width; col++) {
      bool s = this->cols[col].zero_first;
      auto symb = get_next_char(s, index_to_run(i, col));
      c += symb;
      i = lf(col, i, symb);
    }
    return c;
